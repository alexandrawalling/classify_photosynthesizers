...
INPUT_FILE=$(sed -n "$((SLURM_ARRAY_TASK_ID + 1))p" "genome_dirs.txt" | cut -f2)
...
antismash --debug --verbose --output-dir antismash_results/${BASENAME} --cpu 8 --genefinding-tool prodigal "$INPUT_FILE"
